...
//		breakEx();
	...
	public static void randomEx() {
		...
		for(int i = 0; i<rollCnt; i++) {
			//랜덤으로 주사위 값을 추출
			int num = (int)(Math.random()*6)+1; //0.0이상 1.0미만
	...
	public static void breakEx() {
		//1부터 loop를 돌려서 6과 14로 모두 나누어 떨어지는 수는 얼마인가?
		//어디까지 loop를 돌려야 할 지 예측이 쉽지 않으니 while
		int num = 1;
		while(true) {
			if(num%6==0 && num%14==0) {
				break; //루프 탈출
	...
	public static void continueEx() {
		...
		for(int i=1; i<101;i++) {
			//반복문 중간에 continue 
			//남은 구문 실행하지 않고 다음번 루프 진행
			if(i%2==0 || i%3==0) {
				// 아래 남은 코드는 실행되지않고 다음 루프 진행.
				continue;
			}
	...
	public static void forEx() {
		...
		//1. 반복 횟수가 정해져 있을 때.
		//2. 제어 변수를 for문이 해주기 때문에 while 보다 안전하다.
