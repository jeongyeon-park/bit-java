...
//ByteStream
//		이진파일을 입력, 출력하는 클래스 
//		최상위 부모 : InputStream, OutputStream(추상클래스)
public class ByteArrayStreamEx {
	...
	public static void main(String[] args) {
		...
		//순서
		//1. IO 데이터에 적합한 스트림을 열고
		//2. 입력/출력 소스 연결 
		//3. 데이터를 스트림을 통해 입력 or 출력
		
		try {
			//부모 (추상클래스)          (실체 클래스)를 만들어서 부모로 할당받는다.
			InputStream bis = new ByteArrayInputStream(inSrc);
			//오류가 많아서 checked Exception 그래서 예외처리 반드시 해주고 넘어가야한다.
			OutputStream bos = new ByteArrayOutputStream();

			int data = 0; //인풋 스트림으로부터 넘어오는 데이터 저장 변수
			
			//read를 하면 byte데이터를 받아서 int 데이터로 변환하여 리턴
//			data = bis.read();
//			System.out.println("읽은 데이터:"+ data);
			...
			System.out.println("대상배열 : "+Arrays.toString(outSrc));
